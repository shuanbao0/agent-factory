...
| `dept-directive.cjs` | 部门 Chief 指令构建。项目列表展示当前阶段 + 出口条件（来自 `config/project-standards.md`） |
| `sync.cjs` | 项目状态同步（根据 CEO 信号更新阶段/状态） |
| `task-prompt.cjs` | 任务上下文：`buildTaskContext()` 轻量增强 + `buildTaskPrompt()` 完整自包含 prompt。两者均注入 `config/task-standards.md` 中类型专属标准 |
...
除了 Agent 级别的 base-rules，项目和任务各有独立的标准 base 文件：
