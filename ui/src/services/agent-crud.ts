...
export function resolveModelRef(ref: string): string {
  if (!ref) return ref
  try {
...
export function parseSkillMeta(skillMd: string): { name: string; description: string; bins: string[] } {
...
export function generateToolsMd(agentId: string, skills: string[], agentDir: string): string {
  ...
  for (const slug of skills) {
    ...
    if (existsSync(skillMdPath)) {
      try {
        ...
        continue
      } catch (err) { logError('agents-api/parse-skill-md', err) }
    }
...
export function ensureProjectForDepartment(department: string, agentId: string) {
...
export function buildPeersRolesSection(peers: string[]): string {
...
export function generateAgentsMd({ id, role, name, description, peers }: {
...
export async function updateAgent(body: {
  ...
  if (existsSync(agentJsonPath)) {
    try { agentJson = JSON.parse(readFileSync(agentJsonPath, 'utf-8')) } catch (err) { logError('agents-api/read-agent-json', err) }
  }
...
export async function deleteAgent(id: string): Promise<{ ok: boolean; restarted?: boolean; archivedTo?: string | null; error?: string; status?: number }> {
  ...
  return { ok: true, restarted, archivedTo }
}
